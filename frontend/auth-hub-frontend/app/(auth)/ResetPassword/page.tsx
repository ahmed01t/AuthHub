  ...
  if (missingParams) {
    return (
      <div className="w-full max-w-md space-y-8 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 rounded-2xl p-8 text-center space-y-4">
      ...
      {success ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                value={email}
                disabled
                className="w-full pl-12 pr-4 py-3.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-500 dark:text-slate-400 cursor-not-allowed"
              />
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-2">
            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Requirements</p>
            <div className="space-y-1">
              <div className={`flex items-center gap-2 text-sm ${newPassword.length >= 6 ? "text-emerald-600 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400"}`}>
                <div className={`w-1.5 h-1.5 rounded-full ${newPassword.length >= 6 ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"}`} />
                At least 6 characters
              </div>
              <div className={`flex items-center gap-2 text-sm ${confirmPassword && newPassword === confirmPassword ? "text-emerald-600 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400"}`}>
                <div className={`w-1.5 h-1.5 rounded-full ${confirmPassword && newPassword === confirmPassword ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"}`} />
                Passwords match
              </div>
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 disabled:cursor-not-allowed text-white font-semibold rounded-xl shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all duration-200 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white">
          ...
          <div className="space-y-6 max-w-lg">
            <h1 className="text-5xl font-bold leading-tight">
              Secure your <br />
              <span className="text-indigo-200">account.</span>
            </h1>
            <p className="text-lg text-indigo-100 leading-relaxed">
              Create a strong, unique password to keep your AuthHub account safe and protected.
            </p>
